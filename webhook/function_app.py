...
def ping(req: func.HttpRequest) -> func.HttpResponse:
    ...
    headers = [ (key, value) for key, value in req.headers.items() if key.startswith('x-')]
    header_string = json.dumps(headers, indent=2)
    print(f"Headers: {header_string}")
    return func.HttpResponse(status_code=200, body=header_string)


workflow_run_spans : dict[str, trace.Span] = {} # store workflow run spans keyed on f"{run_id}#{run_attempt}"
workflow_run_queued_job_events : dict[str, list] = {} # store workflow run queued job events keyed on f"{run_id}#{run_attempt}"
workflow_run_current_jobs : dict[str, set] = {} # store IDs of currently running jobs for a workflow run keyed on f"{run_id}#{run_attempt}"
workflow_run_can_complete : dict[str, bool] = {} # store if a workflow run can complete keyed on f"{run_id}#{run_attempt}"

workflow_job_spans : dict[str, trace.Span] = {} # store workflow job spans keyed on job_id
...
def process_workflow_job_event(body_json):
    ...
    if action == "queued":
        ...
        if span:
            ...
            end_on_exit = True
            logger.info(f"*** marking to end span for workflow job {id} completed ***")
    else:
        logger.warning(f"Unknown action {action} for workflow job {id}. Payload: {body_json}")

    if not span:
        logger.error(f"Span for workflow job {id} not found. This is unexpected.")
        return
    
    logger.info("### workflow_job event. ID: %s, TraceID: %s, Action: %s", id, span.get_span_context().trace_id, action)
    ...
    if end_on_exit:
        ...
        jobs.discard(id)
        logger.info(f"!!! Workflow run {run_key} has {len(jobs)} jobs running. [discarded {id}]")
        if len(jobs) == 0 and workflow_run_can_complete.get(run_key, False):
            ...
            logger.info(f"All jobs for workflow run {run_key} are complete. Marking run as complete.")
        ...
        logger.info(f"!!! Workflow run {run_key} has {len(jobs)} jobs running. [added {id}]")
    ...
    if event == "workflow_run":
        ...
        if action == "in_progress":
            if run_key in workflow_run_spans:
                logger.info(f"Workflow run span for run_key {run_key} already exists. Ignoring event.")
            ...
            logger.info(f"*** starting span for workflow run {run_key} in_progress ***")
            ...
            if span:
                ...
                if len(jobs) > 0:
                    logger.info(f"Workflow run {run_key} has jobs running ({jobs}). Cannot mark run as complete yet.")
                    workflow_run_can_complete[run_key] = True # indicate that we can complete the run when all jobs are done
                else:
                    complete_span = True
                    logger.info(f"*** marking to end span for workflow run {run_key} {action} ***")
        
        if not span:
            logger.error(f"Span for workflow run {run_key} not found. This is unexpected.")
            return func.HttpResponse(
                f"Span for workflow run {run_key} not found. This is unexpected.",
                status_code=500
            )
        
        logger.info("### workflow_run event. ID: %s, TraceID: %s, Action: %s", run_key, span.get_span_context().trace_id, action)


        with trace.use_span(span, end_on_exit=complete_span) as span:
            if workflow_run_queued_job_events.get(run_key):
                logger.info(f"Processing queued job events for workflow run {run_key}.")
        ...
        workflow_run_span = workflow_run_spans.get(run_key) # TODO - handle not existing
        if not workflow_run_span:
            logger.info(f"Workflow run span for run_id {run_key} not found. Storing event for later processing.")
        ...
        logger.info("### workflow_job event. Run ID: %s, Run TraceID: %s", run_key, workflow_run_span.get_span_context().trace_id)
